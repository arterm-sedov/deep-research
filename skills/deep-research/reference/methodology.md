...
**PROVIDER PRIORITY: Free-first, paid fallback**

Use providers in this order (try each in sequence if previous unavailable):

| Priority | Provider | Cost | Setup Required |
|----------|----------|------|----------------|
| 1 | WebSearch | FREE | None (built-in) |
| 2 | SearXNG | FREE | Docker (optional) |
| 3 | Exa MCP | FREE tier | MCP config (optional) |
| 4 | search-cli | PAID | API keys required |

---

**Option A: Use WebSearch (PRIMARY - always available)**

FREE. Built into the agent. No configuration required.
...
**Option B: Use SearXNG (FREE local search - if running)**

FREE. Tavily-compatible local search via Docker.
...
If available, use searxng-search skill or direct API:
```bash
...
- `include_raw_content` (optional): Include full page text

---

**Option C: Use Exa MCP (FREE tier - if configured)**

FREE tier available. Semantic/neural search capabilities.

If Exa MCP is configured in your environment:
```
...
- `start_published_date` (optional): Filter by date
- `include_domains` (optional): Restrict to specific domains

---

**Option D: Use search-cli (PAID - fallback only)**

Requires paid API keys. Only use if Options A-C unavailable AND user has configured keys.

```bash
search "quantum computing 2025" --json -c 15
```

**This requires paid service APIs. Use free options first.**
...
```
Rate limit encountered → Detect provider failure → Switch to next provider
```
...
All fail → Report limitation, use cached/available sources
```

**Recovery strategy:**
1. **Don't stop research** - Pivot to free alternatives immediately
2. **Document the pivot** - Note in methodology: "Pivoted to [provider] due to rate limits"
3. **Continue momentum** - Use available providers to complete the research
4. **No interruption** - Research continues without waiting for rate limit reset
...
**Rate limit order (try each in sequence):**
1. WebSearch (built-in, no limits)
2. SearXNG (local, no limits)
3. Exa MCP (free tier limits)
4. Tavily / search-cli (paid, strict limits - last resort)
...
**Step 3: Browser Scraping Fallback**

If API-based search and extraction yield insufficient results, use browser automation:

```
API methods exhausted → Check available browser tools
JavaScript content detected → Use browser automation (headless)
CAPTCHA encountered → Switch to headed mode + human handoff
Login required → Human handoff for authentication
```

See [browser-scraping.md](./browser-scraping.md) for detailed instructions.

**Detect available browser tools:**
- Check for playwright-cli skill
- Check for MCP playwright tools (mcp__playwright__*)
- Check for other browser MCPs

**When to use browser scraping:**
- WebFetch returns empty/minimal content (JS not rendered)
- Rate limited by all API providers
- Login-protected content (LinkedIn, Twitter, private Telegram)
- CAPTCHA challenges
- Need to scrape search results directly from DuckDuckGo

**Human handoff protocol:**
1. Detect CAPTCHA/login requirement
2. Open browser in headed mode
3. Notify human: "Please complete the [CAPTCHA/login] to continue. Type 'done' when ready."
4. Wait up to 5 minutes for human confirmation
5. Resume scraping after "done"

**Telegram channels:** Use telegram-scraper skill. Reference [telegram-scraper skill](~/.agents/skills/telegram-scraper/SKILL.md) for Telegram-specific extraction methods.

**Detailed patterns:** See [browser-scraping.md](./browser-scraping.md) for extraction commands, human handoff details, and rate limit considerations.
